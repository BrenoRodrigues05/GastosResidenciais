...
describe('Transacoes - Erros', () => {
  ...
  it('deve mostrar erro no modal com mensagem do backend ao falhar criar', async () => {
  ...
  expect(await within(dialog).findByText('Categoria inválida')).toBeInTheDocument()
...
it('deve usar fallback genérico quando erro não tiver payload', async () => {
  ...
  expect(await within(dialog).findByText(/erro ao criar transação/i)).toBeInTheDocument()
  expect(toastMock).toHaveBeenCalledTimes(0)
})
})
